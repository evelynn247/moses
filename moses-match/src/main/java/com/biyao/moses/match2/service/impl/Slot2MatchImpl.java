 ...
 **/
@Slf4j
@Component(value = MatchStrategyConst.SLOT2)
public class Slot2MatchImpl implements Match2 {
    @Autowired
    ...
    @BProfiler(key = "Slot2MatchImpl.match",
    ...
    public List<MatchItem2> match(MatchParam matchParam) {
        ...
        if(uid != null && uid > 0){
            String redisKey = MatchRedisKeyConstant.MOSES_SLOT2_PREFIX + uid;
            productIds = matchRedisUtil.getString(redisKey);
        }

        //如果uid没有获取到数据，则使用uuid查询
        if(StringUtils.isBlank(productIds)) {
            String redisKey = MatchRedisKeyConstant.MOSES_SLOT2_PREFIX + uuid;
            productIds = matchRedisUtil.getString(redisKey);
        }

        if(StringUtils.isBlank(productIds)){
            log.error("[一般异常][召回源]slot2召回源商品数据为空, uid {}, uuid {}", uid, uuid);
            return result;
        }
        List<ProductScoreInfo> productScoreInfos = StringUtil.parseProductStr(productIds, PID_NUM_MAX_LIMIT, MatchStrategyConst.SLOT2);
        if(CollectionUtils.isEmpty(productScoreInfos)){
            log.error("[严重异常][召回源]解析召回源{}商品数据为空，uid {}, uuid {}", MatchStrategyConst.SLOT2, matchParam.getUid(), matchParam.getUuid());
            return result;
        }
        ...
        for (ProductScoreInfo p : productScoreInfos) {
            ...
            matchItem2.setSource(MatchStrategyConst.SLOT2);
