 ...
 **/
@Slf4j
@Component(value = MatchStrategyConst.IBCF)
public class IbcfMatchImpl implements Match2 {
    @Autowired
    private MatchRedisUtil redisUtil;

    private static  final int PID_NUM_MAX_LIMIT = 200;

    @Autowired
    private ProductDetailCache productDetailCache;

    @BProfiler(key = "IbcfMatchImpl.match",
    ...
    public List<MatchItem2> match(MatchParam matchParam) {
        ...
        String redisKey = MatchRedisKeyConstant.MOSES_IBCF_PREFIX;
        redisKey = redisKey + uuid;

        String productIds = redisUtil.getString(redisKey);
        if(StringUtils.isBlank(productIds)){
            log.error("[一般异常][召回源]ibcf召回源商品数据为空, redisKey {}", redisKey);
            return result;
        }
        List<ProductScoreInfo> productScoreInfos = StringUtil.parseProductStr(productIds, PID_NUM_MAX_LIMIT, MatchStrategyConst.IBCF);
        if(CollectionUtils.isEmpty(productScoreInfos)){
            log.error("[严重异常][召回源]解析召回源{}商品数据为空，uid {}, uuid {}", MatchStrategyConst.IBCF, matchParam.getUid(), matchParam.getUuid());
            return result;
        }
        ...
        for (ProductScoreInfo p : productScoreInfos) {
            ...
            matchItem2.setSource(MatchStrategyConst.IBCF);
