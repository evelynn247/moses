 ...
 **/
@Slf4j
@Component(value = MatchStrategyConst.RTCBR)
public class RtcbrMatchImpl implements Match2 {
    @Autowired
    ...
    @BProfiler(key = "RealTimeMatchImpl.match",
    ...
    public List<MatchItem2> match(MatchParam matchParam) {
        ...
        String redisKey = MatchRedisKeyConstant.MOSES_RTCBR_PREFIX;
        redisKey = redisKey + uuid;

        String productIds = redisUtil.getString(redisKey);
        if(StringUtils.isBlank(productIds)){
            log.error("[一般异常][召回源]rtcbr召回源商品数据为空, redisKey {}", redisKey);
            return result;
        }
        List<ProductScoreInfo> productScoreInfos = StringUtil.parseProductStr(productIds, PID_NUM_MAX_LIMIT, MatchStrategyConst.RTCBR);
        if(CollectionUtils.isEmpty(productScoreInfos)){
            log.error("[严重异常][召回源]解析召回源{}商品数据为空，uid {}, uuid {}", MatchStrategyConst.RTCBR, matchParam.getUid(), matchParam.getUuid());
            return result;
        }
        ...
        for (ProductScoreInfo p : productScoreInfos) {
            ...
            matchItem2.setSource(MatchStrategyConst.RTCBR);
