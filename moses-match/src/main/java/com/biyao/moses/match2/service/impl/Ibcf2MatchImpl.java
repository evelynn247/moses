 ...
 **/
@Slf4j
@Component(value = MatchStrategyConst.IBCF2)
public class Ibcf2MatchImpl implements Match2 {
    @Autowired
    ...
    @BProfiler(key = "Ibcf2MatchImpl.match",
    ...
    public List<MatchItem2> match(MatchParam matchParam) {
        ...
        String redisKey = MatchRedisKeyConstant.MOSES_IBCF2_PREFIX;
        redisKey = redisKey + uuid;

        String productIds = matchRedisUtil.getString(redisKey);
        if(StringUtils.isBlank(productIds)){
            log.error("[一般异常][召回源]ibcf2召回源商品数据为空, redisKey {}", redisKey);
            return result;
        }
        List<ProductScoreInfo> productScoreInfos = StringUtil.parseProductStr(productIds, PID_NUM_MAX_LIMIT, MatchStrategyConst.IBCF2);
        if(CollectionUtils.isEmpty(productScoreInfos)){
            log.error("[严重异常][召回源]解析召回源{}商品数据为空，uid {}, uuid {}", MatchStrategyConst.IBCF2, matchParam.getUid(), matchParam.getUuid());
            return result;
        }
        ...
        for (ProductScoreInfo p : productScoreInfos) {
            ...
            matchItem2.setSource(MatchStrategyConst.IBCF2);
