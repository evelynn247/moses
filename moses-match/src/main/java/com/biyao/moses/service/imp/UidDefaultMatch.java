...
@Slf4j
@Component("UIDM")
public class UidDefaultMatch implements RecommendMatch {
	...
	@BProfiler(key = "com.biyao.moses.service.imp.UidDefaultMatch.executeRecommendMatch",
	...
	public Map<String, List<TotalTemplateInfo>> executeRecommendMatch(String dataKey, MatchDataSourceTypeConf mdst,
		...
		try {
			...
			if (!mdst.isPersonalizedRecommendActSwitch() || StringUtils.isEmpty(matchData)) {
				// 查询UDM无数据，执行兜底方法
				log.error("[一般异常]查询UIDM无数据! uuid {}， dataKey:{}", uuId, dataKey);
				defaultkey = defaultkey.replace(CommonConstants.UIDM_MATCH_NAME, CommonConstants.DEFAULT_MATCH_NAME);
				...
				if (executeRecommendMatch.isEmpty()) {
					log.error("[严重异常]UIDM查询兜底无数据! uuid{}, defaultKey:{}", uuId, defaultkey);
				}
			...
			log.error("[严重异常]UIDM出现异常，uuid {}，dataKey {}", uuId, dataKey, e);
		}
		return resultMap;
