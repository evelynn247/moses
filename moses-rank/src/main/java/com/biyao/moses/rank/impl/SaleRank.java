...
@Slf4j
@Component("saleRank")
public class SaleRank implements RecommendRank {
	
	@Autowired
	ProductDetailCache productDetailCache;

	@BProfiler(key = "SaleRank.executeRecommend", monitorType = {
	...
	public List<TotalTemplateInfo> executeRecommend(RankRequest rankRequest) {
		...
		try {
			...
			log.info("salerank请求参数，uuid={},dataNum={},rankName={},topicId={},sortType={},sortValue={}",uuid,dataNum,rankName,topicId,sortType,sortValue);
			...
			for (TotalTemplateInfo totalTemplateInfo : oriData) {
				if (StringUtils.isNotBlank(totalTemplateInfo.getId())) {
					...
					if (productInfo!=null) {
						Long salesVolume7 = productInfo.getSalesVolume7();
						if (salesVolume7==null) {
							salesVolume7 = 0l;
						}
						totalTemplateInfo.setScore(Double.valueOf(salesVolume7));
					}else{
