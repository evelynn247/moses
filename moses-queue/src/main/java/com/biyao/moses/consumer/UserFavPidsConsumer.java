...
/**
 * @Description 消费收藏消息
 * @date 2019年12月12日下午2:38:16
 * @version V1.0
 * @author 邹立强 (zouliqiang@idstaff.com)
 */
@Component
public class UserFavPidsConsumer extends MqConsumerTemplate {

	private Logger logger = LoggerFactory.getLogger(UserOrderConsumer.class);

	@Value("${rocketmq.server.namesrvAddr}")
	private String namesrvAddr;
	@Value("${rocketmq.user.favorite.product.consumer.group}")
	private String customerGroup;
	@Value("${rocketmq.user.favorite.product.topic}")
	private String topic;
	@Value("${rocketmq.user.favorite.product.tags}")
	private String tags = "*";
	...
	@BProfiler(key = "com.biyao.moses.consumer.UserFavPidsConsumer.handleMessage", monitorType = { MonitorType.TP,
				...
				if (StringUtils.isNotBlank(msgBody)) {
					...
					if (StringUtils.isNotBlank(tp) && StringUtils.isNotBlank(pid) && StringUtils.isNotBlank(time)) {
						...
						if (ub != null && StringUtils.isNotBlank(ub.toString())) {
							...
							msgStr.append("uid=").append(ub.toString()).append("\t").append(UserFieldConstants.FAVPIDS)
									.append("=").append(pid).append(":").append(time);
							sendKafka(producer, producerTopic, msgStr.toString());
						}
					}
				}
			} catch (Exception e) {
				logger.error("消费收藏消息失败:msg={} ,e：{}", messageExt, e);
			}
		}
	}
}