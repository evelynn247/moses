...
public class IdCalculateUtil {
	/**
	 * 生成新的SID SID的生成规则为：${timestamp_ms}-${query_md5_start4}-${uuid_md5_13}，其中： 1.
	 * ${timestamp_ms}是当前时间戳（到毫秒），共13位； 2. ${query_md5_start4}是查询词MD5
	 * 值（32位小写）的前4个字符，共4位； 3. ${uuid_md5_13}是UUID MD5值（32位小写）的前13个字符。 SID共32位长度。
	 */
	public static String createSid(String uuid, String query) {
		...
		return String.format("%d-%s-%s", System.currentTimeMillis(), queryMd5.substring(0, 4),
				uuidMd5.substring(0, 13));
	}
