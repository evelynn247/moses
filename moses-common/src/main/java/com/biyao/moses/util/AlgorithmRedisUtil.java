...
public class AlgorithmRedisUtil {
	 ...
	 * @return
	 */
	public Map<String, String> hscan(String key,int expNum){
		...
		try{
			...
			while (tempResult != null && !ZERO.toString().equals(tempResult.getStringCursor())){
	 ...
	 * @return
	 */
	public ScanResult<Map.Entry<String, String>> hscan(String key,ScanParams scanParams,String cursor){
		try{
			return redisClusterService.hscan(key, cursor, scanParams);
		}catch (Exception e){
			logger.error("[严重异常][redis]redis hscan error, key is {},", key, e);
		}
		return null;
	...
	public Map<String,String> hmgetMap(String key, String[] fields) {
		Map<String,String> result = new HashMap<>();
		try {
			List<String> hmget;
			hmget = redisClusterService.hmget(key, fields);
			...
			for (int i = 0; i < fields.length; i++) {
				result.put(fields[i],hmget.get(i));
			}
		} catch (Exception e) {
			logger.error("redis hmget error, key is {},error message is {}", key, e);
		}
